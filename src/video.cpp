...
{
    godot::UtilityFunctions::print("[gdretro] Video init ", geometry->base_width, " x ", geometry->base_height, " pixel format: ", this->pixel_format);

    this->frame_buffer = godot::Image::create(geometry->base_width, geometry->base_height, false, this->pixel_format);
    godot::UtilityFunctions::print("[gdretro] Video init - OK ");
}

bool GDRetro::core_video_set_pixel_format(unsigned format)
{
    godot::UtilityFunctions::print("[gdretro] core_video_set_pixel_format ", format);
